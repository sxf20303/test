...
#define CHECK_LT(X, Y) { bool RES = (X < Y); assert(RES); }
	...
	#define LOGV(FMT, ...) __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAG, "[%s:%d:%s]:" FMT ,	\
		relativeName(__FILE__), __LINE__, __FUNCTION__, ## __VA_ARGS__)
...
// LOGEE: LOG_Enter_Exit
#ifndef LOGEE
#	define LOGEE LOGD
#endif
#define		ENTER()				LOGEE(" <=")
#define		RETURN(code,type)	{type RESULT = code; LOGEE(" => (%d)", (int)RESULT); return RESULT;}
#define		EXIT()				{LOGEE(" =>"); return;}
#define		PRE_EXIT()			LOGEE(" =>")
